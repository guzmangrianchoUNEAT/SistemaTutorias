...
export default function RegisterScreen({ navigation }: any) {
  ...
  const [loading, setLoading] = useState(false); // Estado para manejar el proceso de registro
  const [error, setError] = useState(''); // Para manejar mensajes de error

  const handleRegister = async () => {
    setError(''); // Reinicia los mensajes de error
  ...
  return (
    ...
    >
      <View style={styles.container}>
        ...
        />
        <FormInput
          placeholder="Confirmar contraseña" // Nuevo campo para confirmar contraseña
          value={confirmPassword}
          onChangeText={setConfirmPassword}
          secureTextEntry
          style={styles.input}
        />
        {error ? <Text style={styles.errorText}>{error}</Text> : null} {/* Muestra el mensaje de error */}
        <Button
